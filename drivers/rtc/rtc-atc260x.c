...
#define RTC_DEFAULT_START_YEAR      (2015)
...
static int atc260x_rtc_settime(struct device *dev, struct rtc_time *tm)
{
	...
	/* Start RTC */
	ret =
	    atc260x_reg_wp_clrpnd(atc260x, atc260x_rtc->regbase + REG_RTC_CTL,
	...
	ret =
	    atc260x_reg_wp_setbits(atc260x, atc260x_rtc->regbase + REG_RTC_CTL,
				   RTC_CTL_ALIP, RTC_CTL_RTCE, RTC_CTL_RTCE);
	if (ret < 0) {
		dev_err(dev, "%s() %u: write reg err, ret=%d\n", __func__,
			__LINE__, ret);
		return ret;
...
static int atc260x_rtc_readalarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	if (century < 19) {
		/* default: 2011 */
		alrm->time.tm_year =
		    RTC_YMD_Y(ymd) + RTC_DEFAULT_START_YEAR - 1900;
	} else {
	...
	ret =
	    atc260x_reg_read(atc260x_rtc->atc260x,
			     atc260x_rtc->regbase + REG_RTC_CTL);
	if (ret < 0) {
		dev_err(dev, "%s() %u: read reg err, ret=%d\n", __func__,
			__LINE__, ret);
		return ret;
...
static int atc260x_rtc_stop_alarm(struct atc260x_rtc_dev *atc260x_rtc)
{
	...
	return atc260x_reg_wp_setbits(atc260x_rtc->atc260x,
				      atc260x_rtc->regbase + REG_RTC_CTL,
...
static int atc260x_rtc_start_alarm(struct atc260x_rtc_dev *atc260x_rtc)
{
	...
	ret =
	    atc260x_reg_wp_clrpnd(atc260x_rtc->atc260x,
				  atc260x_rtc->regbase + REG_RTC_CTL,
				  RTC_CTL_ALIP, RTC_CTL_ALIP);
	if (ret)
		return ret;
...
static int atc260x_rtc_setalarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	if (tm != NULL) {
		dev_info(dev, "%s(): %4d-%02d-%02d %02d:%02d:%02d\n",
			 __FUNCTION__, 1900 + tm->tm_year, tm->tm_mon + 1,
	...
	ret = atc260x_reg_write(atc260x, atc260x_rtc->regbase + REG_RTC_YMDALM,
				RTC_YMD_VAL(year, alrm->time.tm_mon + 1,
					    alrm->time.tm_mday));
	if (ret < 0) {
		dev_err(dev, "%s() %u: write reg err, ret=%d\n", __func__,
			__LINE__, ret);
...
static int atc260x_rtc_suspend(struct device *dev)
{
	...
	ret =
	    atc260x_reg_wp_setbits(atc260x_rtc->atc260x,
				   atc260x_rtc->regbase + REG_RTC_CTL,
...
#if 0
static int check_default_year(struct atc260x_rtc_dev *atc260x_rtc,
			      int default_year)
{
	struct device *dev = atc260x_rtc->rtc->dev.parent;
	struct rtc_time tm;
			      ...
			      int default_year)
{
	...
	atc260x_reg_write(atc260x, atc260x_rtc->regbase + REG_RTC_YMD,
			  RTC_YMD_VAL(year_in_century, RTC_YMD_M(ymd),
				      RTC_YMD_D(ymd)));
...
static int atc260x_rtc_hw_init(struct atc260x_rtc_dev *atc260x_rtc)
{
	...
	if (!(ctl & RTC_CTL_EXT_LOSC_STATUS)) {
		atc260x_reg_wp_setbits(atc260x,
				       atc260x_rtc->regbase + REG_RTC_CTL,
				       RTC_CTL_ALIP,
...
static int atc260x_rtc_probe(struct platform_device *pdev)
{
	...
	atc260x_rtc =
	    devm_kzalloc(&pdev->dev, sizeof(*atc260x_rtc), GFP_KERNEL);
	if (atc260x_rtc == NULL)
		return -ENOMEM;
	...
	ret =
	    devm_request_threaded_irq(&pdev->dev, alm_irq, NULL,
				      atc260x_alm_irq, IRQF_TRIGGER_HIGH,
				      "RTC alarm", atc260x_rtc);
	if (ret != 0) {
	...
	if (ret) {
		dev_err(&pdev->dev,
			"failed to create ext_osc attribute, ret=%d\n", ret);
	}
